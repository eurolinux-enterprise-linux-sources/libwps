...
{
	//! constructor
	explicit CellStyleEntry(shared_ptr<WPSStream> stream) : m_stream(stream), m_parentId(-1), m_entry()
	{
	}
	//! the main strem
...
{
	//! constructor
	GraphicStyle() : m_lineId(0), m_lineStyle(), m_patternId(0), m_extra("")
	{
		for (int i=0; i<4; ++i) m_colorsId[i]=-1;
	}
	//! operator<<
...
struct State
{
	...
	State() :  m_version(-1), m_isUpdated(false), m_idCellStyleMap(), m_idColorStyleMap(),
		...
		m_idFontNameMap(), m_idToParentStyleMap(), m_idToCellStyleEntryMap()
	{
	}
...
bool LotusStyleManager::readGraphicStyleC9(shared_ptr<WPSStream> stream, long endPos)
{
	if (!stream) return false;
	RVNGInputStreamPtr &input = stream->m_input;
	libwps::DebugFile &ascFile=stream->m_ascii;
...
bool LotusStyleManager::readCellStyleE6(shared_ptr<WPSStream> stream, long endPos)
{
	...
	int decalId=int(libwps::readU16(input));
	if (decalId)
	{
		decalId *= 256;
		f << "decal[id]=" << decalId;
...
bool LotusStyleManager::readStyleE6(shared_ptr<WPSStream> stream, long endPos)
{
	if (!stream) return false;
	RVNGInputStreamPtr &input = stream->m_input;
	libwps::DebugFile &ascFile=stream->m_ascii;
