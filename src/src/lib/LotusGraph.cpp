...
struct ZonePc
{
	...
	//! constructor
	explicit ZonePc(shared_ptr<WPSStream> stream) :
		m_type(Unknown), m_isGroup(false), m_groupLastPosition(0), m_numPoints(0), m_vertices(), m_isRoundRect(false), m_stream(stream), m_box(), m_translate(0,0), m_rotate(0), m_arrows(0), m_textBoxEntry(), m_pictureData(), m_pictureHeaderRead(0),
		m_isSent(false), m_extra("")
	{
		for (int i=0; i<2; ++i) m_graphicId[i]=-1;
	}
	//! returns a graphic shape corresponding to the main form (and the origin)
	bool getGraphicShape(WPSGraphicShape &shape, WPSPosition &pos) const;
...
{
	//! constructor
	ZonePcList() : m_zones(), m_groupBeginStack()
	{
	}
...
struct State
{
	...
	State() :  m_version(-1), m_actualSheetId(-1),
		...
		m_sheetIdZonePcListMap(), m_actualZonePc(), m_zIdToSheetIdMap()
	{
	}
...
void LotusGraph::sendGraphics(int sheetId)
{
	...
	if (m_state->m_zIdToSheetIdMap.find(sheetId)!=m_state->m_zIdToSheetIdMap.end())
	{
		...
		if (m_state->m_sheetIdZonePcListMap.find(finalId)!=m_state->m_sheetIdZonePcListMap.end())
		{
			LotusGraphInternal::ZonePcList const &zoneList=m_state->m_sheetIdZonePcListMap.find(finalId)->second;
...
bool LotusGraph::readGraphic(shared_ptr<WPSStream> stream)
{
	...
	case 4:   // arc
	{
		...
		{
			int pts[2];
			for (int j=0; j<2; ++j) pts[j]=int(libwps::readU16(input));
...
bool LotusGraph::readFrame(shared_ptr<WPSStream> stream)
{
	...
	for (int c=0; c<2; ++c)
	{
		...
		for (int i=0; i<2; ++i) pts[i]=int(libwps::readU16(input));
...
void LotusGraph::sendZone(LotusGraphInternal::ZonePcList const &zoneList, size_t id, WPSTransformation &transf)
{
	...
	std::vector<shared_ptr<LotusGraphInternal::ZonePc> > const &zones=zoneList.m_zones;
	if (id>=zones.size())
	{
		WPS_DEBUG_MSG(("LotusGraph::sendZone: can not find the sub zone %d\n", int(id)));
		return;
	}
	shared_ptr<LotusGraphInternal::ZonePc> zone=zones[id];
...
bool LotusGraph::readGraphZone(shared_ptr<WPSStream> stream, int zId)
{
	if (!stream) return false;
	RVNGInputStreamPtr &input = stream->m_input;
	libwps::DebugFile &ascFile=stream->m_ascii;
	...
	case 0x86: // rare, when it exists, end of a set. TODO useme
		...
		if (id==0x86)
		{
			...
			{
				LotusGraphInternal::ZonePcList &current=m_state->m_sheetIdZonePcListMap.find(zId)->second;
	...
	case 0x95: // relative to graphic
		...
		val &=0xFFFC;
		if (val) f << "f0=" << val << ",";
		for (int i=1; i<sz/2; ++i)   // always f0=1, other 0
		{
			val=int(libwps::read16(input));
			if (val) f << "f" << i << "=" << val << ",";
		}
	...
	{
		shared_ptr<LotusGraphInternal::ZonePc> zone(new LotusGraphInternal::ZonePc(stream));
		if (zId<0)
		{
...
bool LotusGraph::readGraphDataZone(shared_ptr<WPSStream> stream, long endPos)
{
	if (!stream) return false;
	RVNGInputStreamPtr &input = stream->m_input;
	libwps::DebugFile &ascFile=stream->m_ascii;
	...
	else if (m_state->m_actualZonePc && m_state->m_actualZonePc->m_type==LotusGraphInternal::ZonePc::Picture)
	{
		...
		if (endPos!=pos)
		{
			...
			if ((readData=input->read(expectedSize, sizeRead)) == 0 || sizeRead!=expectedSize)
			{
