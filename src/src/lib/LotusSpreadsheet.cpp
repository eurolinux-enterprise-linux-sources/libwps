...
struct Extra123Style
{
	...
	{
		for (int i=0; i<2; ++i)
			m_borders[i].m_style=WPSBorder::None;
	}
	//! returns true if the style is empty
	bool empty() const
	{
		for (int i=0; i<2; ++i)
		{
			if (!m_borders[i].isEmpty()) return false;
		}
		return true;
	}
	//! operator==
...
{
	//! constructor
	Table123Styles() : m_defaultCellId(-1), m_rowsToColsToCellIdMap(), m_rowsToColsToExtraStyleMap(), m_rowsToColsToFormatStyleMap()
	{
	}
	...
	void addCellStyle(Vec2i const &cols, Vec2i const &rows, int cellId)
	{
		...
		std::map<Vec2i,int> &map=m_rowsToColsToCellIdMap.find(rows)->second;
	...
	void addCellStyle(Vec2i const &cols, Vec2i const &rows, Format123Style const &format)
	{
		...
		std::map<Vec2i,Format123Style> &map=m_rowsToColsToFormatStyleMap.find(rows)->second;
		if (map.find(cols)!=map.end())
		{
			WPS_DEBUG_MSG(("LotusSpreadsheetInternal::Table123Styles::addCellStyle: find dupplicated cell\n"));
		}
...
struct State
{
	...
	State() :  m_version(-1), m_spreadsheetList(), m_nameToCellsMap(),
		m_rowStylesList(), m_rowSheetIdToStyleIdMap(), m_rowSheetIdToChildRowIdMap(), m_sheetIdToTableStyleMap(),
		m_sheetCurrentId(-1)
	{
	...
	{
		Vec2i pos(-1,id);
		std::map<Vec2i,Table123Styles>::const_iterator cIt=m_sheetIdToTableStyleMap.lower_bound(pos);
	...
	{
		std::map<Vec2i,Table123Styles>::iterator cIt=m_sheetIdToTableStyleMap.lower_bound(pos);
		...
		Table123Styles const &table=cIt->second;
		if (actPos[0]<pos[0])
		{
...
public:
	...
	SubDocument(RVNGInputStreamPtr input, LotusSpreadsheet &sheetParser, WPSEntry const &entry) :
		WKSSubDocument(input, 0), m_sheetParser(&sheetParser), m_entry(entry) {}
	//! destructor
	~SubDocument() {}
...
bool LotusSpreadsheet::readCellsFormat801(shared_ptr<WPSStream> stream, WPSVec3i const &minC, WPSVec3i const &maxC, int subZoneId)
{
	...
	LotusSpreadsheetInternal::Table123Styles *tableStyle=0;
	int const vers=version();
	...
	case 4:   // in general the first one
	{
		...
		int values[4];
		for (int i=0; i<4; ++i) values[i]=int(libwps::readU8(input));
		if ((values[0]&0x80)==0)
			f << "protected[no],";
		else
			values[0] &= 0x7f;
...
void LotusSpreadsheet::sendSpreadsheet(int sheetId)
{
	...
	if (table123Styles)
	{
		for (std::map<Vec2i, std::map<Vec2i,int> >::const_iterator rIt=table123Styles->m_rowsToColsToCellIdMap.begin();
		        rIt!=table123Styles->m_rowsToColsToCellIdMap.end(); ++rIt)
		{
			Vec2i rows=rIt->first;
			newRowSet.insert(rows[0]);
			newRowSet.insert(rows[1]+1);
		}
		for (std::map<Vec2i, std::map<Vec2i,LotusSpreadsheetInternal::Extra123Style> >::const_iterator rIt=table123Styles->m_rowsToColsToExtraStyleMap.begin();
		        rIt!=table123Styles->m_rowsToColsToExtraStyleMap.end(); ++rIt)
		{
			Vec2i rows=rIt->first;
			newRowSet.insert(rows[0]);
			newRowSet.insert(rows[1]+1);
		}
		for (std::map<Vec2i, std::map<Vec2i,LotusSpreadsheetInternal::Format123Style> >::const_iterator rIt=table123Styles->m_rowsToColsToFormatStyleMap.begin();
		        rIt!=table123Styles->m_rowsToColsToFormatStyleMap.end(); ++rIt)
		{
			Vec2i rows=rIt->first;
			newRowSet.insert(rows[0]);
			newRowSet.insert(rows[1]+1);
...
void LotusSpreadsheet::sendRowContent(LotusSpreadsheetInternal::Spreadsheet const &sheet, int row, LotusSpreadsheetInternal::Table123Styles const *table123Styles)
{
	...
	libwps_tools_win::Font::Type const defFontType=m_mainParser.getDefaultFontType();
	...
	if (table123Styles)
	{
		        ...
		        rIt!=table123Styles->m_rowsToColsToCellIdMap.end(); ++rIt)
		{
			...
			for (std::map<Vec2i,int>::const_iterator colIt=rIt->second.begin(); colIt!=rIt->second.end(); ++colIt)
			{
		...
		{
			if (rIt->first[0]>row || rIt->first[1]<row) continue;
			for (std::map<Vec2i,LotusSpreadsheetInternal::Extra123Style>::const_iterator colIt=rIt->second.begin(); colIt!=rIt->second.end(); ++colIt)
			{
		...
		{
			if (rIt->first[0]>row || rIt->first[1]<row) continue;
			for (std::map<Vec2i,LotusSpreadsheetInternal::Format123Style>::const_iterator colIt=rIt->second.begin(); colIt!=rIt->second.end(); ++colIt)
			{
