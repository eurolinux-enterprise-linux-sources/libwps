...
struct State
{
	...
	std::map<int,int> getDataZoneIdToSheetIdMap() const
	{
		...
		std::map<int,int> res;
		for (std::map<int,int>::const_iterator it=m_dataZoneIdToSheetZoneIdMap.begin();
		        it!=m_dataZoneIdToSheetZoneIdMap.end(); ++it)
		{
	...
	std::string getZone1StackDebugName() const
	{
		...
		std::stringstream s;
		s << "ids=[";
		for (size_t i=0; i<m_zone1Stack.size(); ++i)
			s << std::hex << m_zone1Stack[i] << std::dec << ",";
		s << "],";
		return s.str();
	}
	//! the user font type
...
bool LotusParser::readZone1(shared_ptr<WPSStream> stream)
{
	...
	case 0x9: // appear one time at the beginning of the file, pos~8
		...
		int dim[4];
		for (int i=0; i<4; ++i) dim[i]=int(libwps::read32(input));
		f << "dim=" << WPSBox2i(Vec2i(dim[0],dim[1]), Vec2i(dim[2],dim[3])) << ",";
...
bool LotusParser::readSheetZone(shared_ptr<WPSStream> stream)
{
	...
	case 0x86: // one by file in general after sheetC16
		...
		for (int i=0; i<sz/2; ++i)   // 100,0,100,2000 in 123:v4.0 , 100,0,100,0,100:v4.5
		{
			val=int(libwps::readU16(input));
			if (val) f << "f" << i << "=" << std::hex << val << std::dec << ",";
		}
...
bool LotusParser::readMacFontName(shared_ptr<WPSStream> stream, long endPos)
{
	...
	for (int i=0; i<4; ++i)
	{
		int val=(int) libwps::readU8(input); // 0|1
		if (i==1)
		{
			id=val;
			f << "FN" << id << ",";
